...
public class LC1700_NumberOfStudentsUnableToEatLunch {
	...
	public static int countStudents(int[] students, int[] sandwiches) { 
        int[] studentPrefer = new int[2];
        Queue<Integer> stu = new LinkedList<>();
        Queue<Integer> sand = new LinkedList<>();
        for(int i=0; i<students.length; i++) {
        	studentPrefer[students[i]]++;
        	stu.add(students[i]);
        	sand.add(sandwiches[i]);
        }
        
        while(!stu.isEmpty()) {
        	Integer stuVal = stu.peek();
        	Integer snadVal = sand.peek();
        	if(stuVal.equals(snadVal)) {
        		stu.poll();
        		sand.poll();
        		studentPrefer[stuVal]--;
        	}
        	else {
        		if(studentPrefer[0] == stu.size()) return studentPrefer[0];
        		if(studentPrefer[1] == stu.size()) return studentPrefer[1];
        		
        		Integer poll = stu.poll();
        		stu.add(poll);
        	}
        }
        return 0;
