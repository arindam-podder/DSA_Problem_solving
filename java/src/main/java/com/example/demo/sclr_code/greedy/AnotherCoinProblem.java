...
Given an integer A denoting the cost of an item, find and return the smallest number of coins necessary to pay 
exactly the cost of the item (assuming you have a sufficient supply of coins of each of the types you will need).
...
public class AnotherCoinProblem {
	...
	public static void main(String[] args) {
		System.out.println( minimumNumberOfCoin(26) );
	}
	
	public static int minimumNumberOfCoin(int A) {
		ArrayList<Integer> coins = new ArrayList<>();		
		for(int i=0; (int)Math.pow(5, i) <= A; i++) {
			coins.add( (int)Math.pow(5, i) );
		}
