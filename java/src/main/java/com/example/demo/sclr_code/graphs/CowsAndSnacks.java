/*
 * Q2. Cows and snacks (Day 91 - Advanced DSA : Graphs 2: Tological Sort & DSU -- Fri, 27 Oct 2023)
...
There are A snacks flavors, numbered with integers 1,2,…,A. Bessie has A snacks, one snack of each flavor. There are M 
guests and every guest has exactly two favorite flavors. The procedure for eating snacks will go as follows:
...
public class CowsAndSnacks {
	...
	public int solve(int A, int[][] B) {
        ...
        for(int i=0; i<B.length; i++){
            if(union(B[i][0], B[i][1], parent) == false ){
                ans++;
            }
        }
        return ans;
    ...
    public static boolean union(int x, int y, int[] parent){
        int rootX = getParent(x, parent);
        int rootY = getParent(y, parent);
