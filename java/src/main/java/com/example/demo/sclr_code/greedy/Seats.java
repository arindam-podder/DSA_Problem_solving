...
public class Seats {
	...
	public static int minimumJump(String s) {
		int mod = 10000003;
		int n= s.length();
		ArrayList<Integer> position=new ArrayList<>();			//seated man posi
		for(int i=0; i<n; i++) {
			if(s.charAt(i)=='x') {
				position.add(i);
			}
		...
		long ans = 0;
		while(left>=0) {
			ans = (ans + (position.get(mid)-k - position.get(left)) ) % mod ;
			left -=1;
			k +=1;
		}
		k=1;
		while(right<m) {
			ans = (ans + (position.get(right) - position.get(mid)-k) )%mod;
			right++;
			k++;
		}
